...
def _new_batch_dir() -> tuple[str, Path]:
    """生成 batch_id + 目录(YYMMDD_HHMMSS 风格)。"""
    batch_id = time.strftime("%Y%m%d_%H%M%S")
    out = OUTPUT_DIR / f"batch_{batch_id}"
...
def run_batch(
    *,
    ...
    batch_id, batch_dir = _new_batch_dir()
    logger.info("=" * 60)
    logger.info("[batch] 启动 batch_id=%s count=%d domain=%s out=%s",
                batch_id, count, domain, batch_dir)
    logger.info("=" * 60)
    ...
    _emit("started", {"count": count, "domain": domain, "batch_dir": str(batch_dir)})
    ...
    for i in range(1, count + 1):
        ...
        try:
            address_id, email = mail.create_email(domain=domain)
            password = random_password()
            record.update({"email": email, "password": password})
            logger.info("[batch] (%d/%d) 邮箱=%s", i, count, email)
            ...
            # CPA push(失败不影响本地记账;CPA 未启用则 silently 跳过)
